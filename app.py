...
with st.sidebar:
    ...
    groq_key = st.text_input("Groq / OpenAI Key", type="password", placeholder="gsk_... or sk-...")
    if groq_key:
        st.session_state["api_key"] = groq_key
        st.success("LLM key saved ✓")

    el_key = st.text_input("ElevenLabs Key", type="password", placeholder="sk_...")
    if el_key:
        st.session_state["eleven_key"] = el_key
        st.success("ElevenLabs key saved ✓")

    st.markdown("---")
    st.caption("v1.1 · Free infra · Streamlit Cloud")
