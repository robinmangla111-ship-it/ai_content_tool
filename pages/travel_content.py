...
def render():
    ...
    with tab1:
        ...
        with cl:
            ...
            if photo:
                st.image(photo, use_container_width=True, caption="Preview")
    ...
    with tab2:
        ...
        with vr:
            ...
            if gen_vid and v_photos:
                ...
                for i, pf in enumerate(v_photos[:10]):
...
4. Export **1080×1920 MP4** → upload to Shorts / Reels
    ...
    with tab3:
        ...
        with bl:
            b_photo = st.file_uploader("Upload hero photo",
                                        type=["jpg","jpeg","png","webp"], key="b_photo")
            if b_photo:
                st.image(b_photo, use_container_width=True)
            ...
            b_gen = st.button("📦 Generate All Sizes", type="primary",
                              use_container_width=True,
                              disabled=(not b_photo or not b_plats))

        with br:
            st.markdown("### 📋 Results")
            if b_gen and b_photo and b_plats:
