...
def generate_ai_image(prompt: str, width: int, height: int) -> Image.Image | None:
    """
    Generate image via HF InferenceClient (Inference Providers — new API).
    The old api-inference.huggingface.co endpoint returns 410 (gone).
    InferenceClient with provider='auto' routes to fal-ai / replicate automatically.
    Falls back to gradient if no token or quota exceeded.
    """
    hf_key = _hf_key()
    if not hf_key:
        return None
    ...
    try:
        ...
        st.error("huggingface_hub not installed. Add `huggingface_hub>=0.23.0` to requirements.txt")
        return None
    ...
    try:
        ...
        if "401" in err or "unauthorized" in err:
            st.error("❌ HF token invalid (401). Regenerate at huggingface.co → Settings → Access Tokens.")
        elif "402" in err or "payment" in err or "billing" in err:
            st.warning("⚠️ HF Inference Providers require a payment method (pay-per-use, ~$0.003/image).")
            st.info("Add a card at huggingface.co → Settings → Billing — you only pay for what you use.")
        elif "429" in err or "quota" in err or "rate" in err:
            st.warning("⚠️ HF rate limit hit. Wait a minute and try again.")
        elif "503" in err or "loading" in err:
            st.warning("⏳ Model cold-starting — retrying in 20s...")
            time.sleep(20)
            try:
                client2 = InferenceClient(provider="auto", api_key=hf_key)
                img2    = client2.text_to_image(
                    prompt,
                    model="black-forest-labs/FLUX.1-schnell",
                ...
                st.warning(f"Retry failed: {e2}. Using gradient background.")
        else:
            st.warning(f"⚠️ Generation error: {e}. Using gradient background.")
        return None
